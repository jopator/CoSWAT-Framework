    ...
    def setUp(self, demLayer: QgsRasterLayer, channelLayer: QgsVectorLayer, subbasinsLayer: QgsVectorLayer, 
        ...
        if dsNodeIndex >= 0:
            ...
            for point in features:
                ...
                if (self.isHUC or self.isHAWQS) and point[resIndex] > 0:
                    continue
                        ...
                        for lake in lakesLayer.getFeatures():
                            ...
                            if QSWATTopology.polyContains(point.geometry().asPoint(), lakeGeom, lakeRect):
                                if isInlet:
                                    ...
                                    break
                                LAKEID = QSWATTopology._HUCLAKEID if self.isHUC or self.isHAWQS else QSWATTopology._LAKEID
...
#================================================================================================================================================
    ...
    def addLakes(self, lakesLayer: QgsVectorLayer, subbasinsLayer: QgsVectorLayer, chBasinsLayer: QgsVectorLayer, 
        ...
        RES             = QSWATTopology._HUCRES if self.isHUC or self.isHAWQS else QSWATTopology._RES
        lakeResIndex    = lakesProvider.fieldNameIndex(RES)
        
        AREA            = Parameters._AREASQKM if self.isHUC or self.isHAWQS else Parameters._AREA
...
#==================================================================================
# Gets or set ups stuff for channels
#==================================================================================  
        ...
        DSLINKNO            = QSWATTopology._DSLINKNO1 if self.isHUC or self.isHAWQS else QSWATTopology._DSLINKNO

        channelDsLinkIndex  = channelsProvider.fieldNameIndex(DSLINKNO)

        DSNODEID = QSWATTopology._DSNODEID1 if self.isHUC or self.isHAWQS else QSWATTopology._DSNODEID
        ...
        self.addLakeFieldsToChannels(channelsLayer) # It is adding the attribute columns LakeIn, LakeWithin, LakeOut, LakeMain
...
#==================================================================================
# Starts doing shennaningans with lake geometries
#==================================================================================  
...
#===========================================================================================================================================================
# Now it starts looking at channels and assigning roles in relation to lakes !!!   //////////////////////////////////////////////////////////////\\\\\\\\\\\
#===========================================================================================================================================================  
...
# Second pass: Lake Within
            ...
            for channel in channelsProvider.getFeatures():
                ...
                if link not in self.chLinkIntoLake or link not in self.chLinkFromLake or link not in self.chLinkInsideLake:
                    ...
                    # Checks if the lake geometry contains both points, if so, it assigns lake Within.
                    buffer_temp = 0 # 5 meters for weird geometries

                    if QSWATTopology.polyContains(pt1, lakeGeom.buffer(buffer_temp,1), lakeRect) and QSWATTopology.polyContains(pt2, lakeGeom.buffer(buffer_temp,1), lakeRect):
                        lakeData.lakeChLinks.add(link)
                        self.chLinkInsideLake[link] = lakeId
                    ...
                    # Logic using DS & US connection

                        us_links  = [channel[QSWATTopology._USLINKNO1],channel[QSWATTopology._USLINKNO2]]
                        ds_links  = [channel[DSLINKNO]]

                        upstream_connected  = any(us > 0 and (us in self.chLinkInsideLake or us in self.chLinkIntoLake or us in self.chLinkFromLake) for us in us_links)

                        downstream_connected = any(
                            ds > 0 and (ds in self.chLinkInsideLake or ds in self.chLinkIntoLake or ds in self.chLinkFromLake)
                            for ds in ds_links
                        )

                        # More robust logic
                        upstream_within = any(us > 0 and (us in self.chLinkInsideLake)  for us in us_links)
                        upstream_out    = any(us > 0 and (us in self.chLinkFromLake)    for us in us_links)
                        upstream_in     = any(us > 0 and (us in self.chLinkIntoLake)    for us in us_links)

                        downstream_within = any(us > 0 and (us in self.chLinkInsideLake)  for us in ds_links)
                        downstream_out    = any(us > 0 and (us in self.chLinkFromLake)    for us in ds_links)
                        downstream_in     = any(us > 0 and (us in self.chLinkIntoLake)    for us in ds_links)

                        # Case if connected (certain conditions)
                        if (upstream_within and downstream_within) or (upstream_within and downstream_out) or (upstream_in and downstream_within):
                            lakeData.lakeChLinks.add(link)
                            self.chLinkInsideLake[link] = lakeId
                            lakeWithin = lakeId

                        # Case if source inside but connected outside (only to lake within)
                        elif downstream_within and QSWATTopology.polyContains(pt2, lakeGeom.buffer(buffer_temp,1), lakeRect): # Connected donwstream (but outside) and source inside
                            lakeData.lakeChLinks.add(link)
                            self.chLinkInsideLake[link] = lakeId
                            lakeWithin = lakeId


                if lakeIn != 0 or lakeOut != 0:
                    if link in lakeData.lakeChLinks:
                        # Make sure that some already flagged channels are not converted to Within
                        lakeWithin = 0
                        del self.chLinkInsideLake[link]
                        lakeData.lakeChLinks.discard(link)

                    

                lakeAttMap[channelId] = {channelLakeInIndex: lakeIn, channelLakeOutIndex: lakeOut, channelLakeWithinIndex: lakeWithin}
...
# Add lakes ends here  
#=======================================================================================================================================================================
# Add lake Fields to channels starts here
#     
            ...
            if lakeOut > 0:
                ...
                if data.waterRole == QSWATTopology._PLAYATYPE:
                    ...
                    if lakeMain == lakeOut:
                        ...
                        self.pointId += 1
                        point = QgsPointXY(reachData.lowerX, reachData.lowerY) if self.isHUC or self.isHAWQS else QgsPointXY(reachData.upperX, reachData.upperY)
    ...
    def readLakesData(self, db: DBUtils) -> bool:
        ...
        with db.conn as conn:
            if not conn:
                return False
            lakesTable = 'LAKESDATA'
            if not db.hasTableConn(conn, lakesTable):
                return False
        ...
        with self.db.conn as conn:
            ...
            for lake in self.lakesData.values():
                if lake.waterRole in {QSWATTopology._RESTYPE, QSWATTopology._PONDTYPE, QSWATTopology._WETLANDTYPE}:
                    ...
                    if (self.isHUC or self.isHAWQS) and subbasin < 0:
...
#                 routedSubbasins = []
                for channel, SWATChannel in self.channelToSWATChannel.items():
                    ...
                    if channel in outletChannels:
                        pointId = ptId
                    elif self.isHUC or self.isHAWQS:  # subbasins can have multiple channel outlets 
    ...
    def saveOutletsAndSources(self, channelLayer: QgsVectorLayer, outletLayer: Optional[QgsVectorLayer], streamFile: str, useGridModel: bool) -> bool:
        ...
        for reach in channelLayer.getFeatures():
            ...
            if self.outletAtStart:
                ...
                # ignore this error in HAWQS models: sometimes occurs around coasts
                if not useGridModel and not self.isHAWQS and outPt is not None and not QSWATTopology.coincidentPoints(last[-1], outPt, self.xThreshold, self.yThreshold):
            ...
            if linei is None or len(linei) == 0:
                ...
                for j in range(numLines):
                    if i != j:
                        ...
                        if len(linej) > 0 and QSWATTopology.pointOnLine(start, linej[1:], xThreshold, yThreshold):
                            connected = True
                            break
            if not connected:
                return linei
