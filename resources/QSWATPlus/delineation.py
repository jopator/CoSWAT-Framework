...
class Delineation(QObject): 
    ...
    def finishDelineation(self,details: dict) -> None:
        """
        Finish delineation.
    ...
    def addLakes(self,det: dict) -> None:
        ...
        if self._gv.isHUC or self._gv.isHAWQS:
            ...
            if not self._gv.useGridModel and not self.lakePointsAdded and not self._gv.existingWshed:
                ...
                if not self.splitChannelsByLakes(lakesLayer, channelsLayer, demLayer,details=det, reportErrors=reportErrors):
    ...
    def splitChannelsByLakes(self, lakesLayer: QgsVectorLayer, channelsLayer: QgsVectorLayer, demLayer: QgsRasterLayer,details:dict , reportErrors: bool) -> bool:
...
either smaller to lengthen the stream or larger to remove it.  Or if the lake is very small you may need to remove it or enlarge it."""
        ...
        #==========================================================================================================================================================================
            ...
            # //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            # HERE IT SAVED THE NEW POINT ID AS INLET OR OUTLET IN THE TOPO
            if res == 0:
                self._gv.topo.lakeInlets[lakeId].add(currentPointId)
...
#==============================================================================================================================================================================
# Getting snap layer (THE POINTS), creating lakeInlets and lakeOutlets dictionary


        # Gets fields from snap layer
...
# Snap thingies ends
#==============================================================================================================================================================================
# Modification to lake geometries
...
#We will use snap points that have been created on a separate script with a more robust logic and will be read here directly.
#The file will be created and provided during the framework: lakes_resolved_snap-{region}-{auth}-{code}.shp

        snapLakesFile = variables.resolved_snaps.format(**details)

        print(f'\t \t > Snap File:   {snapLakesFile}',flush=True)


        lakePointMap = defaultdict(list)
        
        pointLayer = QgsVectorLayer(snapLakesFile,"SnapPoints","ogr")

        print(f"Point layer valid: {pointLayer.isValid()}", flush=True)
        print(f"Feature count: {pointLayer.featureCount()}", flush=True)
        
        print('\t \t \t - Reading points.....',flush=True)


        pointProvider = pointLayer.dataProvider()
        pointFeatures = list(pointProvider.getFeatures())


        for pointFeat in pointLayer.getFeatures():
            lake_id = pointFeat['lakeId']
            lakePointMap[lake_id].append(pointFeat)
        ...
        for lake in lakeProvider.getFeatures():
            ...
            # Get geometry and set dictionaries    
            ...
            # Iterate over snap points provided on file and get the ones that intersect the lake
            # print(f'\t \t \t Reading for Lake ID: {lakeId}...',flush=True)

            for pointFeat in lakePointMap.get(lakeId, []):
                ...
                feat.setAttribute(idIndex, currentPointId)
                feat.setAttribute(resIndex, pointFeat['RES'])  # Assume 'res' gives 0 (inlet) or 1 (outlet)
                feat.setAttribute(inletIndex, 0)
                feat.setAttribute(srcIndex, 0)
                feat.setAttribute(addIndex, 1)
                feat.setGeometry(pointGeom)

                newPoints.append(feat)


                # Add point to dictionaries
                if pointFeat['RES'] == 0:
                    self._gv.topo.lakeInlets[lakeId].add(currentPointId)
                else:
                    self._gv.topo.lakeOutlets[lakeId].add(currentPointId)
            ...
            #     if QSWATTopology.disjointBoxes(box, lineBox): # Box is bbox for lake perimeter
            #         continue
...
#=====================================================================================================================================================================================
# Logical loop for multipoint intersection
...
#                         if QSWATTopology.polyContains(outlet, geom, box): # If the lake's bounding box contains the outlet (Why geom??)
                            
#                             if QSWATTopology.polyContains(source, geom, box): # If it also contains the source --> Internal
...
#                                 QSWATUtils.information(                    # If it does not contain the source --> Starts and ends outside of the lake (is clipped on the first crossing  point)
# """Channel with LINKNO {0} crosses the  boundary of lake {1}
...
#                             currentPointId = makeNewPoint(points, source, outlet, 
#                                                           line, newPoints, currentPointId, 
#                                                           0, chLink, lakeId, fields, transform)         # Creates new point on the first intersection starting from source

#                         else:

#                             if QSWATTopology.polyContains(source, geom, box):                           # If the lake's bounding box contains the source, but not the outlet
...
#                                 currentPointId = makeNewPoint(points, source, outlet,                    # Creates new point on the first intersection starting from outlet
#                                                               line, newPoints, currentPointId, 
#                                                               res, chLink, lakeId, fields, transform)
                                


#                             else:                                                                        # It does not contain either the source or the outlet but still intersects
#                                 crossingChannels.append((channel, points))                               # Appended to a list of crossing channels


# Logical loop for multipoint intersection ends there
#=====================================================================================================================================================================================
# Single point intersection

# Quite simple; checks if outlet is inside or not and then defines which res value to use to see if the intersection is inlet or outlet.
...
# Single point intersection ends there
#=====================================================================================================================================================================================
            ...
            if numOutlets == 0 and len(crossingChannels) > 0:
                ...
                # It creates an outlet from the crossing channels if no other outlet was found 
#=====================================================================================================================================================================================
# Message to indicate nothing will be done about these channels that cross, but start and end outside
...
# Re running Taudem
    ...
    def identifyLakes(self, lakesLayer: QgsVectorLayer) -> Tuple[int, bool, bool]:
        ...
        # for i, field in enumerate(lakeProvider.fields()):
        #     print(i, field.name())
        ...
        if lakeIdIndex < 0:
            ...
            for lake in lakeProvider.getFeatures(request):
                lakeId = lake[lakeIdIndex]
                val = lake[lakeResIndex]
                # print(f"lake {lakeId}: RES={val}, type={type(val)}", flush=True)
...
#                 QSWATUtils.loginfo(line)
    ...
    def runTauDEM2(self, ver = None, reg = None, in_outlet_path = None, Mask_gpd = None, sel_file = None, outletsExist2 = False) -> None:
        ...
        if not self._dlg.useOutlets.isChecked():
            ...
            self.runTauDEM(outletFile, True, reg = reg, ver = ver, in_outlet_path = in_outlet_path, Mask_gpd = Mask_gpd, sel_file = sel_file, outletsExist1=outletsExist1)
    ...
    def runTauDEM(self, outletFile: Optional[str], makeWshed: bool, ver = None, reg = None, in_outlet_path = None, Mask_gpd = None, sel_file = None, outletsExist1 = False) -> None:
        """Run TauDEM."""
        self.delineationFinishedOK = False
        ...
        if self._dlg.useOutlets.isChecked():
            ...
            if not outletsExist1:
                os.system(f'python3 create-outlets.py {ver} {reg}')
            import geopandas

            outlets_all         = geopandas.read_file(in_outlet_path)
            outlets_sel         = geopandas.clip(outlets_all, Mask_gpd)
            outlets_sel.to_file(sel_file)
...
#                                                      linknoField, dslinknoField, orderField)
    ...
    def createWatershedShapefile(self, wFile: str, subbasinsFile: str, ft: FileTypes, root: QgsLayerTree) -> None:
        ...
        for row in range(numRows):
            ...
            shapes.addRow(wBuffer.reshape([np.int32(numCols)]), row)
        shapes.finish()
