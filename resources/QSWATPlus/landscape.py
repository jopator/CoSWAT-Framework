...
# Top-level worker function for multiprocessing (must be outside class to avoid pickling issues)
def _clipRasterSimple(args):
    """Simplified worker function for parallel raster clipping that avoids QGIS objects."""
    inFile, clipperFile, outFile, noData = args
    import processing
    from pkg_resources import parse_version
    ...
    if os.path.exists(outFile):
        inStat = os.path.getmtime(inFile) if os.path.exists(inFile) else 0
        clipStat = os.path.getmtime(clipperFile) if os.path.exists(clipperFile) else 0
        outStat = os.path.getmtime(outFile)
        if outStat >= inStat and outStat >= clipStat:
            return outFile, False  # Already up to date
    ...
    return outFile, True  # New file created
...
#                         QSWATUtils.error(err, self._gv.isBatch)
    ...
    def calcFloodplainParallel(self, useInversion, root, nProc=None):
        """Generate floodplain raster using parallel processing for the computational loops only."""
        if nProc is None:
            nProc = min(mp.cpu_count(), 4)  # Default to 4 max to avoid memory issues
            
        QSWATUtils.loginfo(f'Starting floodplain calculation with {nProc} parallel processes for computation')
        print(f'    - starting floodplain calculation with {nProc} parallel processes for computation')
        self._dlg.setCursor(Qt.WaitCursor)
        
        # Do raster clipping sequentially to avoid QGIS object pickling issues
        if useInversion:
            # Generate inverted flow accumulation 
            time1 = time.process_time()
            ridgesResult = self.calcRidges()
            if ridgesResult is None:
                return
            ...
            # Sequential raster clipping (to avoid pickling issues)
            ridgep, isNew1 = self.clipRaster(d8Inv, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew1
            ridges, isNew2 = self.clipRaster(accInv, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew2
            
            subbasins = distances = slopeDir = flowAcc = None
        else:
            base, ext = os.path.splitext(self._gv.demFile)
            ad8File = base + 'ad8' + ext
            
            # Sequential raster clipping (to avoid pickling issues)
            subbasins, isNew1 = self.clipRaster(self._gv.basinFile, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew1
            distances, isNew2 = self.clipRaster(self._gv.distStFile, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew2
            slopeDir, isNew3 = self.clipRaster(self._gv.pFile, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew3
            flowAcc, isNew4 = self.clipRaster(ad8File, self.clipperFile, root)
            self.mustRun = self.mustRun or isNew4
            
            ridgep = ridges = None
            
        # Use parallel version for floodplain computation
        self.FP = Floodplain(self._gv, self._progress, self.chunkCount)
        self.FP.runParallel(self.demRaster, self._gv.valleyDepthsFile, self.ridgeThresh, self.floodThresh, self.branchThresh, subbasins, distances, slopeDir, flowAcc, ridgep, ridges, self.noData, self.mustRun, nProc)
        self._dlg.setCursor(Qt.ArrowCursor)
        ...
        for row in range(self.numRows):
            for col in range(self.numCols):
                if self.channelsRaster.read(row, col) != channelsNoData:
                    (val, elev, path, ok) = self.drainsTo(row, col)
                    if not ok:
                        return False
    ...
    def drainsTo(self, row, col):
        """Return drainage value, valley floor elevation and drainage path for (row, col), plus boolean flag indicating OK or error."""
        path = []
        while True:
            ...
            if val >= 0:  # already done this point
                ...
                return (val, self.demRaster.read(row, col) - self.valleyDepthsRaster.read(row, col), path, True)
            val, elev = self.sides.get((row, col), (-1, 0))
            if val > 0:  # found the headwater or side drainage point
                path.append((row, col))
                return (val, elev, path, True)
            if self.channelsRaster.read(row, col) > 0:
                if len(path) > 0:
