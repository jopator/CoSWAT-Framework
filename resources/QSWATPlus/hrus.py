    ...
    def addReservoirs(self) -> None:
        ...
        for basinData in self.basins.values():
            for channel, channelData in basinData.getLsus().items():
                ...
                if waterBody is not None and not waterBody.isUnknown(): # is reservoir, pond or wetland
                    # need to remove WATR HRUs
                    # SWATChannel = self._gv.topo.channelToSWATChannel[channel]
                    # if SWATChannel == 761:
                    #     QSWATUtils.loginfo('Waterbody area = {0}'.format(lsuData.waterBody.area))
                    #     QSWATUtils.loginfo('cropsoilslope area = {0}'.format(lsuData.cropSoilSlopeArea))
                    #     hrusArea = 0
                    #     for hru, celldata in lsuData.hruMap.items():
                    #         QSWATUtils.loginfo('HRU {0} has area {1}'.format(hru, celldata.area))
                    #         hrusArea += celldata.area
                    #     QSWATUtils.loginfo('Total HRUs area = {0}'.format(hrusArea))
                    # lsuData.removeWaterHRUs(self._gv.db.waterLanduse)
                    # if SWATChannel == 761:
                    #     QSWATUtils.loginfo('After removing water HRUs:')
                    #     QSWATUtils.loginfo('Waterbody area = {0}'.format(lsuData.waterBody.area))
                    #     QSWATUtils.loginfo('cropsoilslope area = {0}'.format(lsuData.cropSoilSlopeArea))
                    #     hrusArea = 0
                    #     for hru, celldata in lsuData.hruMap.items():
...
#===============================================================================
        ...
        for basin, basinData in self.basins.items():
            for channel, channelData in basinData.getLsus().items(): 
                for landscape, lsuData in channelData.items():
                    ...
                    if exemptWater:
                        hrusArea -= lsuData.waterBody.originalArea
                        if abs(lsuData.cropSoilSlopeArea - lsuData.waterBody.originalArea) < self._gv.cellArea:  # all reservoir or pond
                            lsuData.hruMap = dict()
                            lsuData.cropSoilSlopeNumbers = dict()
                            continue
        ...
        for basin, basinData in self.basins.items():
            for channel, channelData in basinData.getLsus().items(): 
                for landscape, lsuData in channelData.items():
                    ...
                    if areaToRedistribute > 0:
                        # just to make sure we don't divide by zero or close to it
                        if abs(hrusArea - areaToRedistribute) < self._gv.cellArea:
                            if exemptWater: # all rest is reservoir or pond
                                lsuData.hruMap = dict()
                                lsuData.cropSoilSlopeNumbers = dict()
                    ...
                    for (crop, soilSlopeNumbers) in lsuData.cropSoilSlopeNumbers.items():
                        ...
                        if soilAreaToRedistribute > 0:
                            ...
                            if abs(cropArea - soilAreaToRedistribute) < self._gv.cellArea:
                                if exemptWater: # all rest is reservoir or pond
                                    lsuData.hruMap = dict()
                                    lsuData.cropSoilSlopeNumbers = dict()
                                    continue
                    ...
                    for (crop, soilSlopeNumbers) in lsuData.cropSoilSlopeNumbers.items():
                        for (soil, slopeNumbers) in soilSlopeNumbers.items():
                            ...
                            if slopeAreaToRedistribute > 0:
                                ...
                                # just to make sure we don't divide by zero
                                if abs(soilArea - slopeAreaToRedistribute) < self._gv.cellArea:
                                    if exemptWater: # all rest is reservoir or pond
                                        lsuData.hruMap = dict()
                                        lsuData.cropSoilSlopeNumbers = dict()
                                        continue
    ...
    def removeSmallHRUsbyTarget(self) -> None:
        ...
        one or more landscape units to have no HRUs.
        ...
        QSWATUtils.loginfo('Num to remove: {0}'.format(numToRemove))
        # print('targetVal1: {0}'.format(self.targetVal))
        # print('HRUs count: {0}'.format(self.countHRUs()))
        # print('Possible removals: {0}'.format(len(removals)))
        # print('Num to remove: {0}'.format(numToRemove))
        # numToRemoveStart = numToRemove
        for nextItem in removals:
            if numToRemove == 0:
                break
            if self.removeHru(nextItem[0], nextItem[1], nextItem[2], nextItem[3], nextItem[4], nextItem[5], nextItem[6]):
                numToRemove -= 1
        # print('Number removed: {0}'.format(numToRemoveStart - numToRemove))
    ...
    def removeHru(self, basin: int, hru: int, channel: int, landscape: int, crop: int, soil: int, slope: int) -> None:
        ...
        # if channel in self._gv.topo.chLinkToWater and lsuData.waterBody is None and (landscape == QSWATUtils._NOLANDSCAPE or landscape == QSWATUtils._FLOODPLAIN):
        #     # need to create an empty water body since identified as pond or reservoir in inlets/outlets file
        #     # but no WATR pixels found.  May be reported later as zero area
        #     wid, _, role = self._gv.topo.chLinkToWater[channel]
        #     channelData = self._gv.topo.channelsData[channel]
        #     waterBody = WaterBody(0, 0, channelData.lowerZ, channelData.lowerX, channelData.lowerY)
        #     lsuData.waterBody = waterBody
        #     waterBody.id = wid
        ...
        #     else:
        #         waterBody.setPond()
        # water body needs to be preserved if it exists and has an identified role
        exemptWater = lsuData.waterBody is not None and not lsuData.waterBody.isUnknown()
        hrusArea = lsuData.area
        if exemptWater:
            hrusArea -= lsuData.waterBody.originalArea
        ...
        if hru not in hrus:
            SWATBasin = self._gv.topo.subbasinToSWATBasin[basin]
            SWATChannel = self._gv.topo.channelToSWATChannel[channel]
            cropStr = self._gv.db.landuseCodes[crop]
            soilStr = self._gv.db.soilNames[soil]
            slopeStr = self._gv.db.slopeRange(slope)
            QSWATUtils.loginfo('HRU {1} not found in basin {0} channel {2} landscape {3} crop {4} soil {5} slope {6}'.format(SWATBasin, hru, SWATChannel, landscape, cropStr, soilStr, slopeStr))
            return False
        areaToRedistribute = hrus[hru].area
        lsuData.removeHRU(hru, crop, soil, slope)
        # if len(lsuData.hruMap) == 0:
        #     QSWATUtils.loginfo('LSU area for channel {0} is {1}'.format(self._gv.topo.channelToSWATChannel.get(channel, -1), lsuData.area))
        #     QSWATUtils.loginfo('HRUs area {0}'.format(hrusArea))
        #     QSWATUtils.loginfo('Area to redistribute {0}'.format(areaToRedistribute))
        if areaToRedistribute > 0:
...
#                     fw.writeLine('Areas of landuses, soils{0} exclude ponds and reservoirs.'.format(hruString))
    ...
    def printLandscapeHRUs(self, SWATBasin: int, SWATChannel: int, channelData: Dict[int, LSUData], basinHa: float, subHa: float, fw: fileWriter, curs: Any) -> None:
        ...
        for landscape, lsuData in channelData.items():
            ...
            if lsuData.waterBody is None or lsuData.waterBody.isUnknown() or lsuData.cropSoilSlopeArea > lsuData.waterBody.originalArea: # else all water
    ...
    def printLSUHRUs(self, lsuId: int, SWATChannel: int, landscape: int, lsuData: LSUData, channelData: Dict[int, LSUData], 
        ...
        if waterBody is not None and routeWaterAsWaterBody:
            ...
            if arlsuHa > 0:
                ...
                fw.write(locale.format_string('%.2F', percent3).rjust(23))
            fw.writeLine('')
            # for actual reservoir print total area (using area rather than originalArea)
            if waterBody.isReservoir():
                if waterBody.cellCount > 0:
    ...
    def hruArea(self, hru: str, lsuData: LSUData) -> float:
        ...
        try:
            if hru.startswith('RES') or hru.startswith('PND'):
                return cast(float, lsuData.waterBody.originalArea)
            else:
                hruNum = int(hru)
                ...
                for channel, channelData in basinData.getLsus().items():
                    ...
                    for landscape, lsuData in channelData.items():
                        ...
                        if waterBody is not None and not waterBody.isUnknown():
                            ...
                            SWATBasin = self._gv.topo.subbasinToSWATBasin[basin]
                            area = waterBody.area / 1E4 # area in ha, using merged area rather than originalArea
...
#         reservoirsFileN, _ = QSWATUtils.nextFileName(reservoirsFile, 0)
    ...
    def basinWaterBodiesArea(self, basinData: BasinData) -> float:
        ...
        result = 0.0
        for channelData in basinData.getLsus().values():  # channel, channelData in basinData.getLsus().items():
            for lsuData in channelData.values():
                if lsuData.waterBody is not None and not lsuData.waterBody.isUnknown():
                    result += lsuData.waterBody.originalArea
                    # SWATChannel = self._gv.topo.channelToSWATChannel[channel]
                    # QSWATUtils.loginfo('Channel {0} has reservoir or pond area {1}, originally {2}'.
                    #                    format(SWATChannel, result / 10000, lsuData.waterBody.originalArea / 10000))
        return result
...
#===============================================================================
    ...
    def setAreaPercentChoice(self) -> None:
        ...
        if self.CreateHRUs.isArea:
            ...
            if minHRUs <= int(self.CreateHRUs.targetVal) <= numHRUs:
    ...
    def readProj(self) -> None:
        ...
        if soilLayer is not None:
            ...
            self.soilFile = soilFile
            self.soilLayer = soilLayer
        self._db.useSTATSGO, found = proj.readBoolEntry(self._gv.attTitle, 'soil/useSTATSGO', False)
        if found and self._db.useSTATSGO:
            self._dlg.STATSGOButton.setChecked(True)
        self._db.useSSURGO, found = proj.readBoolEntry(self._gv.attTitle, 'soil/useSSURGO', False)
        ...
        plantSoilDatabase, found = proj.readEntry(self._gv.attTitle, 'soil/database', '')
        ...
        useLandscapes, found = proj.readBoolEntry(self._gv.attTitle, 'lsu/useLandscapes', False)
        if found:
            self._gv.useLandscapes = useLandscapes
        useLeftRight, found = proj.readBoolEntry(self._gv.attTitle, 'lsu/useLeftRight', False)
        if found:
            self._gv.useLeftRight = useLeftRight
        floodFile, found = proj.readEntry(self._gv.attTitle, 'lsu/floodplainFile', '')
        if found:
            floodFile =  proj.readPath(floodFile)
            if os.path.exists(floodFile):
                self._gv.floodFile = floodFile
        reservoirThresholdNoLandscape, _ = proj.readNumEntry(self._gv.attTitle, 'lsu/thresholdResNoFlood', 101)
        reservoirThresholdFloodplain, _ = proj.readNumEntry(self._gv.attTitle, 'lsu/thresholdResFlood', 101)
        if useLandscapes:
        ...
        channelThresholdArea, found = proj.readNumEntry(self._gv.attTitle, 'delin/thresholdCh', 0)
        if found and channelThresholdArea > 0:
            self._gv.channelThresholdArea = channelThresholdArea * self._gv.cellArea
        channelMergeByPercent, _ = proj.readBoolEntry(self._gv.attTitle, 'lsu/channelMergeByPercent', True)
        if channelMergeByPercent:
            self._dlg.channelPercentButton.setChecked(True)
        else:
            self._dlg.channelAreaButton.setChecked(True)
        channelMergeVal, _ = proj.readNumEntry(self._gv.attTitle, 'lsu/channelMergeVal', 0)
    ...
    def saveEntries1(self, proj: QgsProject) -> None:
        ...
        proj.writeEntry(self._gv.attTitle, 'landuse/file', proj.writePath(self.landuseFile))
        proj.writeEntry(self._gv.attTitle, 'soil/file', proj.writePath(self.soilFile))
        proj.writeEntry(self._gv.attTitle, 'landuse/table', self.landuseTable)
        proj.writeEntry(self._gv.attTitle, 'landuse/plant', self._db.plantTable)
        proj.writeEntry(self._gv.attTitle, 'landuse/urban', self._db.urbanTable)
        proj.writeEntry(self._gv.attTitle, 'landuse/water', self._db.waterLanduse)
        proj.writeEntry(self._gv.attTitle, 'soil/table', self.soilTable)
        proj.writeEntry(self._gv.attTitle, 'soil/useSTATSGO', self._db.useSTATSGO)
        proj.writeEntry(self._gv.attTitle, 'soil/useSSURGO', self._db.useSSURGO)
        proj.writeEntry(self._gv.attTitle, 'soil/database', proj.writePath(self._db.plantSoilDatabase))
        proj.writeEntry(self._gv.attTitle, 'soil/databaseTable', self._db.usersoilTable)
        proj.writeEntry(self._gv.attTitle, 'lsu/useLandscapes', self._gv.useLandscapes)
        if self._gv.useLandscapes:
            proj.writeEntry(self._gv.attTitle, 'lsu/thresholdResFlood', self._dlg.reservoirThreshold.value())
        else:    
            proj.writeEntry(self._gv.attTitle, 'lsu/thresholdResNoFlood', self._dlg.reservoirThreshold.value())
        proj.writeEntry(self._gv.attTitle, 'lsu/useLeftRight', self._gv.useLeftRight)
        proj.writeEntry(self._gv.attTitle, 'lsu/floodplainFile', proj.writePath(self._gv.floodFile))
        proj.writeEntry(self._gv.attTitle, 'lsu/channelMergeByPercent', self._dlg.channelPercentButton.isChecked())
        proj.writeEntry(self._gv.attTitle, 'lsu/channelMergeVal', self._dlg.channelMergeSlider.value())
    ...
    def saveEntries2(self, proj: QgsProject) -> None:
        ...
        proj.writeEntry(self._gv.attTitle, 'hru/slopeBands', QSWATUtils.slopesToString(self._db.slopeLimits))
        proj.writeEntry(self._gv.attTitle, 'hru/slopeBandsFile', proj.writePath(self._gv.slopeBandsFile))
