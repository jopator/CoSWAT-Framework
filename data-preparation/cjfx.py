...
def show_progress(progress, end, dt=None, string_before="", string_after="", bar_length=100, precision = 1, d_count = None, scroll_text = None):
    ...
    if platform.system() != "Windows":
        try:
            ...
            term_width = 80  # fallback when no terminal (batch jobs, redirected output, etc.)

        if (term_width - (len(string_after) + 21)) < bar_length:
            bar_length = term_width - (len(string_after) + 21)
