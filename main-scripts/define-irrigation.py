...
if __name__ == '__main__':
    ...
    for region in regions:
        ...
        # Shapes
        lakes_path = 'Watershed/Shapes/lakes-grand-ESRI-54003.shp'
        chans_path = 'Watershed/Shapes/dem-aster-ESRI-54003-lakeBurntchannel.shp'
        ...
        # Read and process
        lakes_gdf   = gpd.read_file(f'{model_dir}/{lakes_path}')
        ...
        # Groundwater 
        irr_grw_hrus_gdf = hrus_gdf[hrus_gdf['irr_grw']==1]
        ...
        hru_data.write(name='hru-data.hru updated by CoSWAT+ framework after define-irrigation.py',overwrite=True)
        ...
        # add new rows
        tmpl_row = (landuse_lum_df.loc[landuse_lum_df['name'] == 'agrl_lum']
            .iloc[0]               # Series (1-D)
            .to_dict())
        ...
        * Irrigate 25 mm with sprinkler if water stress < 0.8
        ...
        for index, row in res_irr_hrus.iterrows():
            ...
            res_list = [int(float(x)) for x in res_list_str]

            # Get fractions
            source_res_gdf = lakes_gdf[lakes_gdf['LakeId'].isin(res_list)]
            ...
            lum_dtl.add_dtl(dtl_name,1,1,actions,conds_df,acts_df,overwrite=True)
        ...
        * Irrigate 25 mm with sprinkler if water stress < 0.8
        ...
        for index, row in riv_irr_hrus.iterrows():
            ...
            lum_dtl.add_dtl(dtl_name,1,1,1,conds_df,acts_df,overwrite=True)
