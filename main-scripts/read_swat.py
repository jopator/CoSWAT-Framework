...
# import dask.dataframe as dd Not used now to avoid version issues with other libraries
...
class swatModelFile(): #This is a table object intended to read model files (e.g. Hydrology.res)
    ...
    def write(self,name:str,overwrite = True ,write_path = "Null"):
        ...
        trailing_cols = ['obj_typ', 'obj_id', 'hyd_typ','frac']
        for col in trailing_cols:
            if col in df.columns:
                if df[col].dtype == object:
                    df[col] = df[col].fillna("")
                else:
                    # convert to object type first, then fill
                    df[col] = df[col].astype(object).where(df[col].notna(), "")
        float_cols = df.select_dtypes(include="float").columns.difference(trailing_cols)
        ...
        with open(write_path, "w") as file:
            file.write(f"{name}: written by read_swat - Jose Teran for CoSWAT model\n")
            file.write(df.to_string(index=False, justify="left", col_space=10))
...
class swat_Dtl(): #This is a decision table file
    ...
    def add_dtl(self, name: str, conds: int, alts: int, acts: int,
                cond_table, action_table, replace=False, overwrite=False, write_path="null"):
        '''
        ...
        while i < len(new_lines):
            if new_lines[i].strip().startswith("name") and i + 1 <len(new_lines):
                
                parts = new_lines[i+1].split()
                
                if parts:
                    name_indices.append((i, new_lines[i + 1].split()[0].strip()))

                i += 1
            i += 1
            
        existing_names = [n for _,n in name_indices] 

        if name in existing_names:
            if replace:
                print(f"Replacing decisition table: {name}".ljust(50), end='\r')
                
                match_idx = next(idx for idx,n in name_indices if n == name)
                next_name_indices = [idx for idx, n in name_indices if idx > match_idx]
                end_idx = next_name_indices[0] if next_name_indices else len(new_lines)                
                del new_lines[match_idx:end_idx]
                
            else:
                print(f"Decision table '{name}' already exists. Skipping.".ljust(50), end='\r')
                return
